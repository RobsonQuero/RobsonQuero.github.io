...
function Loja (canvas,player){
	
	//Estados das telas do Jogo
	var gameState = 0;
	var thisScene = 4; 	// Tela atual;
	...
	//Trilha Sonora
	trilha_Loja = new Audio();
	trilha_Loja.src = "musicas/InsideHouse.mp3";
	trilha_Loja.volume = 0.5;
	
	
	//Variavel como o sprite do mapa
	var scene = new Image();
	scene.src = "img/cenarios/loja.png";
	...
	npc.setSprite("img/NPCs/npc_vendedor.png");
	npc.transform(312, 297, 16, 32); 			// objeto.tamanho do objeto(Posição X, Posição Y, Largura, Altura);
	npc.interact(250, 267, 105, 107);
	npc.msgInteract(["img/layouts/falas/npc_vendedor_msg.png"], 1, 115, 0, 500, 100);
	
	
	//Itens da Loja
	var lifePotion = new Objetos(null);
	lifePotion.setSprite("img/itens/pocao.png");
	lifePotion.transform(291, 329, 14, 16); 


	var magicPotion = new Objetos(null);
	magicPotion.setSprite("img/itens/mana.png");
	magicPotion.transform(308, 329, 14, 16); 
	

	var shield = new Objetos(null);
	shield.setSprite("img/itens/escudo.png");
	shield.transform(325, 329, 14, 16); 

	
	var sword = new Objetos(null);
	sword.setSprite("img/itens/espada.png");
	sword.transform(342, 322, 16, 32); 
	...
	this.update = function (){
		...
		nextScene = interacoes.interacaoTeleport(portaSaida, 413, 275, 0, 0, gameState, 3, buttonA);
		gameState = nextScene;
		
		
		nextScene = interacoes.fimMsgTeleport(npc, player.posX, player.posY, player.srcX, player.srcY, gameState, 8);
		gameState = nextScene;
