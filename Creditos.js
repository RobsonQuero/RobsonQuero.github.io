...
function Creditos(canvas){
	
	//Estados das telas do Jogo
	var gameState = 0;
	var thisScene = 2; 	// Tela atual;
	...
	//Sprite do Mapa
	var scene = new Image();
	scene.src = "img/cenarios/creditos.png";
	...
	function mouseClicked(e){
		if(gameState == thisScene){
			...
			if(buttonLeftMenu <= mouseX && mouseX <= buttonRightMenu && buttonTopMenu <= mouseY && mouseY <= buttonDownMenu){
				//alert("Menu");
				//console.log("PASSOU PELO fim dos creditos!");
				nextScene = 1; // gameState = MENU
				gameState = nextScene;
			}
		}
	}
